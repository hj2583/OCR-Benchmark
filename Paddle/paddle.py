...
# Force-disable oneDNN/PIR before any Paddle import to avoid Paddle runtime
# compatibility issues on Windows CPU.
...
def get_structure_engine_class():
    ...
    # Running this file directly puts its directory first on sys.path, so the
    # file name "paddle.py" can shadow the installed Paddle package.
...
def run_local_table_extraction(doc_path: str, summary_out: str):
    ...
    output_dir = os.path.dirname(os.path.abspath(summary_out))
    ...
    json_data = _build_json_output(page_outputs, page_sizes)

    with open(json_out, "w", encoding="utf-8") as f:
        json.dump(json_data, f, indent=2, ensure_ascii=False)

    markdown_layout = _render_high_fidelity_document_markdown(page_outputs, page_sizes)
    with open(summary_out, "w", encoding="utf-8") as f:
        f.write(markdown_layout)

    readable_layout = _render_readable_document_markdown(page_outputs)
    with open(readable_out, "w", encoding="utf-8") as f:
        f.write(readable_layout)

    html_content = _render_json_to_html(json_data)
    with open(html_out, "w", encoding="utf-8") as f:
        f.write(html_content)

    print(f"[SUCCESS] {text_cfg['success_prefix']}")
    print(f"  -> OCR layout markdown saved to: {summary_out}")
    print(f"  -> OCR readable markdown saved to: {readable_out}")
    print(f"  -> OCR JSON output saved to: {json_out}")
    print(f"  -> OCR HTML output saved to: {html_out}")
